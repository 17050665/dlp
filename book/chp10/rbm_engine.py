...
class Rbm_Engine(object):
            ...
            with tf.Session() as sess:
                ...
                for epoch in range(self.num_epochs):
                    ...
                    for idx, batch in enumerate(batches):
                        cost_val, _, _, _ = sess.run([self.cost, self.w_upd8, self.bh_upd8, self.bv_upd8], feed_dict={
                                self.X: batch, 
                                self.hrand: np.random.rand(batch.shape[0], self.num_hidden), 
                                self.vrand: np.random.rand(batch.shape[0], batch.shape[1])})
    ...
    def build_model(self):
        ...
        self.hrand = tf.placeholder(shape=[None, self.num_hidden], dtype=tf.float32, name='h')
        self.vrand = tf.placeholder(shape=[None, self.n], dtype=tf.float32, name='v')
        self.y = tf.placeholder(shape=[None, 10], dtype=tf.float32, name='y')
        self.keep_prob = tf.placeholder(dtype=tf.float32, name='keep_prob')
        #
        self.W = tf.Variable(tf.truncated_normal(shape=[self.n, self.num_hidden], mean=0.0, stddev=0.1), name='W')
        self.bh_ = tf.Variable(tf.constant(0.1, shape=[self.num_hidden]), name='bh')
        ...
        loss = - tf.reduce_mean(tf.add(
                        ...
                        tf.multiply(tf.subtract(1.0, self.X), tf.log(clip_sup))))
        self.cost = loss + self.regcoef*(tf.nn.l2_loss(self.W) + tf.nn.l2_loss(self.bh_) + tf.nn.l2_loss(self.bv_))
